...
class NfoSceneParser:
    ...
    def __init__(self, stash):
        ...
        self._all_movies_cache = None  # 缓存全量 Movie 列表（reload 模式下复用）
        ...
        if self._stash.get_mode() == "reload" and config.reload_tag:
            ...
            for tag in (results or {}).get("tags", []):
    ...
    def __prepare(self, scene_id):
        ...
        if not self._scene:
            log.LogError(f"Could not find scene with id: {scene_id}")
        self._folder_data = {}
        self._file_data = {}
    ...
    def __update(self):
        ...
        if updated_scene is not None and updated_scene.get("id") == str(self._scene_id):
            log.LogInfo(
                f"Successfully updated scene: {self._scene_id} using '{self._file_data.get('file')}'")
        else:
            log.LogError(
    ...
    def __find_create_scene_data(self):
        ...
        scene_performer_ids = list(
            map(lambda p: p.get("id"), self._scene.get("performers") or []))
        scene_tag_ids = list(map(lambda t: t.get("id"), self._scene.get("tags") or []))
        ...
        bl = config.blacklist
        scene_data = {
            "source": self._file_data.get("source"),
            "title": (self.__build_scene_title() or self._scene.get("title") or None) if "title" not in bl else None,
            "details": (self._file_data.get("details") or self._scene.get("details") or None) if "details" not in bl else None,
            "date": (self._file_data.get("date") or self._scene.get("date") or None) if "date" not in bl else None,
            "rating": (self._file_data.get("rating") or self._scene.get("rating") or None) if "rating" not in bl else None,
            "urls": (self._file_data.get("urls") or self._scene.get("urls") or None) if "urls" not in bl else None,
            ...
            "scene_index": (self._file_data.get("scene_index") if file_movie_id else None) or scene_movie_index or None,
            "cover_image": (self._file_data.get("cover_image") or None) if "cover_image" not in bl else None,
        }
        return scene_data

    def __build_scene_title(self):
        title = self._file_data.get("title")
        if not title:
            return None
        # Append version suffix to distinguish different versions of the same movie
        title_suffix = self._file_data.get("title_suffix")
        if title_suffix:
            title = f"{title} {title_suffix}"
        scene_index = self._file_data.get("scene_index")
        if scene_index:
            title = f"{title} CD{scene_index}"
        return title
    ...
    def __find_create_performers(self):
        ...
        for actor in self._file_data["actors"]:
            ...
            if not performers or not performers.get("performers"):
                log.LogDebug(f"No performers found for '{actor}'")
            match_direct = False
            match_alias = False
            matching_id = None
            matching_name = None
            ...
            for performer in (performers or {}).get("performers", []):
    ...
    def __find_create_studio(self) -> str:
        ...
        if not studios or not studios.get("studios"):
            log.LogDebug(f"No studios found for '{self._file_data['studio']}'")
        match_direct = False
        match_alias = False
        matching_id = None
        match_count = 0
        # 1st pass for direct name matches
        for studio in (studios or {}).get("studios", []):
    ...
    def __find_create_tags(self):
        ...
        if not all_tags or not all_tags.get("tags"):
            log.LogDebug("No tags found in stash")
        for file_tag in self._file_data["tags"]:
            ...
            for tag in (all_tags or {}).get("tags", []):
                if self.__is_matching(file_tag, tag["name"], True):
                    ...
                    match_count += 1
            # 2nd pass for alias matches
            if not matching_id and config.search_tag_aliases:
                for tag in (all_tags or {}).get("tags", []):
                    if tag.get("aliases"):
    ...
    @staticmethod
    def __normalize_movie_name(name):
        """规范化 Movie 名用于比较：NFKC + 波浪号统一 + 小写 + 去除空格"""
        if not name:
            return ""
        # NFKC 规范化：全角/半角统一（如 Ａ→a，９→9，～→~）
        name = unicodedata.normalize("NFKC", name)
        # 统一波浪号：U+301C (〜 WAVE DASH) → ~ (NFKC 后 U+FF5E 已转为 ~)
        name = name.replace("\u301c", "~")
        return name.lower().replace(" ", "")

    def __find_create_movie(self, studio_id):
        movie_name = self._file_data.get("movie")
        if not movie_name:
            return
        movie_id = None
        base_name = self._file_data.get("base_name")
        norm_movie_name = self.__normalize_movie_name(movie_name)

        # Step 1: Search existing Movies by movie_name and base_name (Stash INCLUDES)
        # movie_name 可能是长字符串（NFO title），导致 Stash INCLUDES 搜索失败
        # base_name 是文件名前缀（如 "IDBD-287"），更短更可靠
        search_terms = [movie_name]
        if base_name and base_name.lower() != movie_name.lower():
            search_terms.append(base_name)
        for search_term in search_terms:
            movies = self._stash.gql_findMovies(search_term)
            for movie in (movies or {}).get("movies", []):
                # 规范化匹配：去除空格后比较，支持 "S1GIRLSCOLLECTION" 匹配 "S1 GIRLS COLLECTION"
                if self.__normalize_movie_name(search_term) in self.__normalize_movie_name(movie["name"]):
                    log.LogDebug(
                        f"Matched existing movie '{movie['name']}' with id {movie['id']} (search: '{search_term}')")
                    return movie["id"]

        # Step 2: 全量搜索 Movie，规范化精确匹配
        # 兜底：Stash INCLUDES 搜索可能因空格分词等问题失败，全量获取后 Python 端规范化匹配
        if self._all_movies_cache is None:
            self._all_movies_cache = self._stash.gql_findAllMovies()
        for movie in self._all_movies_cache:
            if norm_movie_name == self.__normalize_movie_name(movie.get("name", "")):
                log.LogDebug(
                    f"Matched existing movie '{movie['name']}' with id {movie['id']} (normalized full scan)")
                return movie["id"]

        # Step 3: Search scenes by base_name (title/path) to find an associated Movie
        if base_name:
            for search_func, search_type in [
                (self._stash.gql_findScenesByTitle, "title"),
                (self._stash.gql_findScenesByPath, "path"),
            ]:
                scenes_result = search_func(base_name)
                for scene in (scenes_result or {}).get("scenes", []):
                    if scene.get("movies"):
                        log.LogDebug(
                            f"Found existing movie '{scene['movies'][0]['movie']['name']}' via scene {search_type} search for '{base_name}'")
                        return scene["movies"][0]["movie"]["id"]

        # Step 4: Create a new movie when it does not exist
        if not config.create_missing_movies or config.dry_mode:
            log.LogInfo(
                f"'{movie_name}' movie creation prevented by config")
        else:
            ...
            if new_movie:
                ...
                log.LogInfo(
                    f"Created missing movie '{movie_name}' with id {new_movie['id']}")
                # 更新缓存，避免同进程后续处理重复创建
                if self._all_movies_cache is not None:
                    self._all_movies_cache.append(
                        {"id": movie_id, "name": movie_name})
            else:
                log.LogError(f"Failed to create movie '{movie_name}'")
        return movie_id
